...
def test_get_repositories_success():
    """Test get_repositories parses gh output correctly."""
    mock_run = MagicMock()
    mock_run.stdout = '[{"name": "repo1"}, {"name": "repo2"}]'
    ...
    with patch('renovate_config_cli.cli.run_command', return_value=mock_run) as mock_run_cmd:
        ...
        mock_run_cmd.assert_called_with(['gh', 'repo', 'list', 'owner', '--json', 'name', '--limit', '1000', '--no-archived'])
...
def test_main_single_repo():
    ...
    with patch('argparse.ArgumentParser.parse_args',
               return_value=argparse.Namespace(owner="owner", repo="repo", preset="preset", no_pr=False)), \
...
def test_main_all_repos():
    ...
    with patch('argparse.ArgumentParser.parse_args',
               return_value=argparse.Namespace(owner="owner", repo=None, preset="preset", no_pr=False)), \
         patch('renovate_config_cli.cli.check_gh_cli', return_value=(True, "")), \
        ...
        mock_get_repos.assert_called_once_with("owner")
