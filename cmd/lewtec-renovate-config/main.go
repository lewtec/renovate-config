...
var validRepoNameRegex = regexp.MustCompile(`^[a-zA-Z0-9_.-]+$`)
...
func isValidInput(input string) bool {
	...
	return validRepoNameRegex.MatchString(input)
}
	...
	var rootCmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if !isValidInput(owner) {
				slog.Error("Invalid owner name", "owner", owner)
				os.Exit(1)
			}
			...
			if len(args) == 2 {
				repo := args[1]
				if !isValidInput(repo) {
					slog.Error("Invalid repo name", "repo", repo)
					os.Exit(1)
				}
